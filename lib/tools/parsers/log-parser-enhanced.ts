...
function detectAnomalies(
  ...
  Object.entries(pattern.anomalyFields).forEach(([field, fieldConfig]) => {
    ...
    if (values.length < config.minSamples) {
      return;
    }

    if (fieldConfig.type === 'http_status') {
          ...
          if (statusCounts[criticalStatus] > 0) {
            ...
            if (percentage > 5) {
              anomalies.push({
                ...
                lineNumber:
                  entries.find((e) => parseInt(String(e[field])) === criticalStatus)?._lineNumber ||
                  0,
              });
            }
          }
    ...
    } else if (fieldConfig.type === 'log_level') {
      ...
      if (durations.length > 0) {
        ...
        values.forEach((v, i) => {
          const duration = parseFloat(String(v));
    ...
    } else if (fieldConfig.type === 'ip_address') {
  ...
  return {
    ...
    timeRange:
      bucketValues.length > 0
        ...
        : null,
    hourlyDistribution: hourlyValues.length > 0 ? hourlyStats : null,
    peakHours:
