...
import { unescapeXML } from '@/lib/tools';
import {
  UploadIcon,
  LinkIcon,
  SampleIcon,
...
// Sample escaped XML for demonstration
const SAMPLE_ESCAPED_XML = `&lt;?xml version=&quot;1.0&quot;?&gt;
&lt;root&gt;
...
export default function XMLUnescapePage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const result = await unescapeXML(input);
        setOutput(result);
      } catch (err) {
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] overflow-hidden bg-background">
      ...
      <div className="px-4 py-3 border-b border-border/30">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-medium text-text-primary">XML Unescape</h1>
            <span className="text-sm text-text-tertiary">
              Convert XML entities back to characters
            </span>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="text-xs text-text-secondary font-medium">
              {loading ? 'Processing...' : input ? 'Ready' : 'Enter escaped XML to unescape'}
            </span>
          </div>
          {error && (
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <div className="px-4 py-2 border-b border-border/30 bg-surface-secondary/10">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-text-secondary">Unescaped XML</span>
            ...
            <textarea
              ...
              placeholder={
                input
                  ? loading
                    ? 'Processing...'
                    : 'Unescaped XML will appear here'
                  : 'Unescaped XML will appear here'
              }
            />
          </div>
