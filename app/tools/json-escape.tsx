...
import { escapeJSON } from '@/lib/tools';
import { SAMPLE_JSON_STRING } from '@/lib/tools/shared';
import {
  UploadIcon,
  LinkIcon,
  SampleIcon,
...
export default function JSONEscapePage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const result = await escapeJSON(input);
        setOutput(result);
      } catch (err) {
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] overflow-hidden bg-background">
      ...
      <div className="px-4 py-3 border-b border-border/30">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-medium text-text-primary">JSON Escape</h1>
            <span className="text-sm text-text-tertiary">Escape strings for JSON format</span>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="text-xs text-text-secondary font-medium">
              {loading ? 'Processing...' : input ? 'Ready' : 'Enter text to escape for JSON'}
            </span>
          </div>
          {error && (
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <div className="px-4 py-2 border-b border-border/30 bg-surface-secondary/10">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-text-secondary">JSON Escaped</span>
            ...
            <textarea
              ...
              placeholder={
                input
                  ? loading
                    ? 'Processing...'
                    : 'JSON escaped text will appear here'
                  : 'JSON escaped text will appear here'
              }
            />
          </div>
