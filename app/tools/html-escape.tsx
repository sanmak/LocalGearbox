...
import { SAMPLE_HTML_ENTITIES } from '@/lib/tools/shared';
import {
  UploadIcon,
  LinkIcon,
  SampleIcon,
...
export default function HTMLEscapePage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const result = await escapeHTML(input);
        setOutput(result);
      } catch (err) {
      ...
      <div className="px-4 py-3 border-b border-border/30">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-medium text-text-primary">HTML Escape</h1>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="text-xs text-text-secondary font-medium">
              {loading ? 'Processing...' : input ? 'Ready' : 'Enter HTML to escape'}
            </span>
          </div>
          {error && (
