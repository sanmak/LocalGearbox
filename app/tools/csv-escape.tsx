...
import { escapeCSV } from '@/lib/tools';
import { SAMPLE_CSV_DATA } from '@/lib/tools/shared';
import {
  UploadIcon,
  LinkIcon,
  SampleIcon,
...
export default function CSVEscapePage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const result = await escapeCSV(input);
        setOutput(result);
      } catch (err) {
      ...
      <div className="px-4 py-3 border-b border-border/30">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-medium text-text-primary">CSV Escape</h1>
            <span className="text-sm text-text-tertiary">Escape text for safe CSV format</span>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="text-xs text-text-secondary font-medium">
              {loading ? 'Processing...' : input ? 'Ready' : 'Enter CSV data to escape'}
            </span>
          </div>
          {error && (
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0">
        ...
        <div className="flex-1 flex flex-col min-h-0 border-b lg:border-b-0 lg:border-r border-border/30">
          <div className="px-4 py-2 border-b border-border/30 bg-surface-secondary/10">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-text-secondary">Input CSV Data</span>
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <div className="px-4 py-2 border-b border-border/30 bg-surface-secondary/10">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-text-secondary">CSV Escaped</span>
            ...
            <textarea
              ...
              placeholder={
                input
                  ? loading
                    ? 'Processing...'
                    : 'CSV escaped data will appear here'
                  : 'CSV escaped data will appear here'
              }
            />
          </div>
