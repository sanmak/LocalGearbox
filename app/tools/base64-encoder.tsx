...
import { encodeBase64 } from '@/lib/tools';
import { SAMPLE_TEXT } from '@/lib/tools/shared';
import {
  UploadIcon,
  LinkIcon,
  SampleIcon,
...
export default function Base64EncoderPage() {
  ...
  const handleFileImport = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // For binary files, read as data URL then convert to base64
  ...
  const handleExport = useCallback(() => {
    if (!output) {
      setError('No encoded data to export');
      return;
    }
    ...
    try {
      ...
      a.href = url;
      a.download = `base64-encoded-${new Date().toISOString().split('T')[0]}.txt`;
      ...
      <div className="px-4 py-3 border-b border-border/30">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-medium text-text-primary">Base64 Encoder</h1>
          ...
          <div className="flex items-center gap-1">
            <input ref={fileInputRef} type="file" onChange={handleFileImport} className="hidden" />
            <button
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="text-xs text-text-secondary font-medium">
              {loading ? 'Processing...' : input ? 'Ready' : 'Enter text or upload file to encode'}
            </span>
          </div>
          {error && (
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0">
        ...
        <div className="flex-1 flex flex-col min-h-0 border-b lg:border-b-0 lg:border-r border-border/30">
          <div className="px-4 py-2 border-b border-border/30 bg-surface-secondary/10">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-text-secondary">Input Text</span>
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <div className="px-4 py-2 border-b border-border/30 bg-surface-secondary/10">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-text-secondary">Base64 Encoded</span>
              <div className="flex items-center gap-2">
                ...
                {output && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-2 py-1.5 text-text-tertiary hover:text-text-primary hover:bg-surface-secondary/50 rounded transition-colors text-xs"
                    title="Export file"
                  >
                    📁
                  </button>
