...
import { unescapeHTML } from '@/lib/tools';
import {
  UploadIcon,
  LinkIcon,
  SampleIcon,
...
// Sample escaped HTML for demonstration
const SAMPLE_ESCAPED_HTML = `&lt;p&gt;Hello &amp; welcome to our &quot;website&quot;!&lt;/p&gt;
&lt;div class=&quot;container&quot;&gt;
  &lt;h1&gt;Title with &lt;em&gt;emphasis&lt;/em&gt;&lt;/h1&gt;
  &lt;p&gt;Content with &amp;copy; symbol&lt;/p&gt;
&lt;/div&gt;`;
...
export default function HTMLUnescapePage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const result = await unescapeHTML(input);
        setOutput(result);
      } catch (err) {
      ...
      <div className="px-4 py-3 border-b border-border/30">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-medium text-text-primary">HTML Unescape</h1>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="text-xs text-text-secondary font-medium">
              {loading ? 'Processing...' : input ? 'Ready' : 'Enter escaped HTML to unescape'}
            </span>
          </div>
          {error && (
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0">
        ...
        <div className="flex-1 flex flex-col min-h-0 border-b lg:border-b-0 lg:border-r border-border/30">
          <div className="px-4 py-2 border-b border-border/30 bg-surface-secondary/10">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-text-secondary">Escaped HTML</span>
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <div className="px-4 py-2 border-b border-border/30 bg-surface-secondary/10">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-text-secondary">Unescaped HTML</span>
            ...
            <textarea
              ...
              placeholder={
                input
                  ? loading
                    ? 'Processing...'
                    : 'Unescaped HTML will appear here'
                  : 'Unescaped HTML will appear here'
              }
            />
          </div>
